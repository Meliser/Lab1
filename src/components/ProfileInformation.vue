...
<b-container id="CProfileInformation" fluid class="d-flex flex-column
  ...
  <b-container fluid id="user-info" class="d-flex align-items-center flex-column"
  ...
  <div v-else id="user-not-found">
    <b-alert show variant="danger" v-if="load_error">
      Пользователя не существует.
    </b-alert>
  </div>
  <b-row class="w-100" align-h="center">
    <b-col sm="12" md="8" lg="6">
      <b-form-input id="user-input" fluid v-model="user_id"
...
export default {
  ...
  methods: {
    async loadUser(userID) {
      let userResponse = {};
      await fetchJsonp(`https://api.vk.com/method/users.get?lang=0&user_ids=${userID}&fields=has_photo,photo_max_orig,online,status&access_token=${process.env.VUE_APP_VK_KEY}&v=5.103`)
        .then((response) => response.json()).then((json) => {
          userResponse = json;
        });

      if (userResponse.error || !(Object.keys(userResponse).length > 0)) {
  ...
  watch: {
    async user_id(userID) { return this.loadUser(userID); },
  },
};
</script>
